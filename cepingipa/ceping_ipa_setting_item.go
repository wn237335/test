...
type ceping_ipa_setting_item struct {
...
func Ipasettingitem1() {
	...
	for _, i6 := range cepinguser {
		var ceping_ipa_setting_item1 []ceping_ipa_setting_item
		err = db.Table("ceping_ipa_setting_item").Where("user_id = ?", i6.Id).Find(&ceping_ipa_setting_item1).Error
		if err != nil {
			fmt.Println(err)
			continue
		}
		if ceping_ipa_setting_item1 == nil {
			continue
		}
		...
		for _, i2 := range jss {
			if i2.(map[string]interface{})["type"] == "iOS" {
				result[i2.(map[string]interface{})["item"].(string)] = 1
			}
		}

		for _, i7 := range ceping_ipa_setting_item1 {
		...
		var ceping_ipa_setting_item2 user_item
		...
		ceping_ipa_setting_item2.UserId = aps_user.Id
		ceping_ipa_setting_item2.Name = i6.Username
		re, _ := json.Marshal(result)
		ceping_ipa_setting_item2.Item = string(re)
		ceping_ipa_setting_item2.Type = "iOS"
		//将数据存入表中
		err = db2.Table("user_item").Create(&ceping_ipa_setting_item2).Error
		if err != nil {
			fmt.Println(err)
		}
...
func Ipasettingitem() {
	...
	//db.LogMode(true) //开启sql debug 模式
	var ceping_ipa_setting_item1 []ceping_ipa_setting_item
	err = db.Table("ceping_ipa_setting_item").Find(&ceping_ipa_setting_item1).Error
	if err != nil {
		fmt.Println(err)
	}
	...
	judge := -1
	ii := -1
	for i, i2 := range ceping_ipa_setting_item1 {
		var ceping_ad_setting_item3 []ceping_ipa_setting_item
		if judge == -1 {
			judge = i2.UserId
			ii = i
		}
		...
		err2 := db.Table("ceping_ipa_setting_item").Where("user_id = ?", judge).Find(&ceping_ad_setting_item3).Error
		if err2 != nil {
