...
type ceping_ipa_task struct {
	Id int `json:"id"`
	AppFileMd5 string `bson:"app_file_md5"`
	ApkShieldtype string `json:"apk_shieldtype"`
	AppFilename string `bson:"app_filename"`
...
type CPTaskMeta struct {
	...
	ChannelFilePath  string `bson:"channel_file_path"`  //渠道文件地址[加固]
	SubmitType       string `bson:"submit_type"`        //提交方式[加固][API、WEB]
	PackFile         string `bson:"pack_file" `         //加固后路径
...
func Ipatask() {
	...
	sspmysql := utils.Sspmysql()
	bbmysql := utils.Bangbangmysql()

	db, err := gorm.Open("mysql", bbmysql)
	if err != nil {
		fmt.Println(err)
	}
	...
	db2, err := gorm.Open("mysql", sspmysql)
	if err != nil {
		fmt.Println(err)
	}
	...
	for _, i2 := range ceping_ipa_task1 {
		...
		//把对应任务的回调数据存入mongodb
		fmt.Println(23443)
		Ipataskresult1(i2.Id, result, i2.RequestId, i2.FailMsg)
		break
	}
}