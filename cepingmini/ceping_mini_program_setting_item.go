...
type ceping_mini_program_setting_item struct {
...
func Miniprogramsettingitem1() {
	...
	for _, i6 := range cepinguser {
		var ceping_mini_program_setting_item1 []ceping_mini_program_setting_item
		err = db.Table("ceping_mini_program_setting_item").Where("user_id = ?", i6.Id).Find(&ceping_mini_program_setting_item1).Error
		if err != nil {
			fmt.Println(err)
			continue
		}
		if ceping_mini_program_setting_item1 == nil {
			continue
		}
		...
		for _, i2 := range jss {
			if i2.(map[string]interface{})["type"] == "MiniProgram" {
				result[i2.(map[string]interface{})["item"].(string)] = 1
			}
		}

		for _, i7 := range ceping_mini_program_setting_item1 {
		...
		var ceping_mini_program_setting_item2 user_item
		...
		ceping_mini_program_setting_item2.UserId = aps_user.Id
		re, _ := json.Marshal(result)
		ceping_mini_program_setting_item2.Item = string(re)
		ceping_mini_program_setting_item2.Name = i6.Username
		ceping_mini_program_setting_item2.Type = "MiniProgram"
		//将数据存入表中
		err = db2.Table("user_item").Create(&ceping_mini_program_setting_item2).Error
		if err != nil {
			fmt.Println(err)
		}
